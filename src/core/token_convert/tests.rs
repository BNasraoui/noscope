...
fn centralized_provider_output_to_scoped_token_computed_expiry() {
    ...
    let output =
        crate::ports::provider_exec::parse_provider_output(r#"{"token": "secret"}"#, 3600).unwrap();
...
fn provider_output_expires_at_provided_flag_accessible() {
    ...
    let result = super::provider_output_to_scoped_token_with_metadata(output, "role", None, "prov");
